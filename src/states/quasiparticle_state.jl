...
function Base.convert(::Type{RightGaugedQP}, input::LeftGaugedQP{S}) where {S<:InfiniteMPS}
    ...
    rBs[1] = rBE
    for i in len:-1:2
        rBE = TransferMatrix(input.left_gs.AL[i], input.right_gs.AR[i]) *
              rBE *
              exp(1im * input.momentum)
        rBs[i] += rBE
    end
...
function Base.convert(::Type{<:FiniteMPS}, v::QP{S}) where {S<:FiniteMPS}
    ...
    function simplefuse(temp)
        frontmap = isomorphism(storagetype(temp),
                               fuse(space(temp, 1) * space(temp, 2)),
                               space(temp, 1) * space(temp, 2))
        backmap = isomorphism(storagetype(temp),
